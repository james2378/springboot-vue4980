<template>
	<div class="addEdit-block" :style='{"padding":"30px 30px 0"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="地名" prop="diming">
					<el-input v-model="ruleForm.diming" placeholder="地名" clearable  :readonly="ro.diming"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else class="input" label="地名" prop="diming">
					<el-input v-model="ruleForm.diming" placeholder="地名" readonly></el-input>
				</el-form-item>
			</template>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `diming/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dimingCrossAddOrUpdateFlag = false;
						 this.parent.search();
